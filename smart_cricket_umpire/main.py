...
INPUT_VIDEO_NAME = "lbw_test3.mp4"
...
while vid.isOpened():
    ...
    print(f"Detected classes: {[int(det.cls[0]) for det in detections]}")  # Debugging class IDs
    ...
    # # Track ball and handle deviation detection
...
#     # Draw OBBs (Oriented Bounding Boxes) manually
#     frame = frame.copy()
#     obb_data = results[0].obb
#     if obb_data and obb_data.xywhr is not None:
#         for i in range(len(obb_data.xywhr)):
#             cls_id = int(obb_data.cls[i])
#             name = model.names[cls_id]
#             color = colors(cls_id, True)
#             x, y, w, h, angle = obb_data.xywhr[i].cpu().numpy()
...
#             cv2.putText(frame, name, (int(x), int(y)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
